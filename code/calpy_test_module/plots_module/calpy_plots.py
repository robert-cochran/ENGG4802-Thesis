...
def sounding_pattern_plot(
    ...
    for plot_row in range(num_subplot_rows):
        ...
        #AupB
        conditions = plots_support.ranges_satisfying_condition( A, B, *name_to_edge_condition["AupB"] )
        for L,R in conditions:
            AA[0:2, L:R+1] = colours.index('lawngreen')

        #BupA
        conditions = plots_support.ranges_satisfying_condition( A, B, *name_to_edge_condition["BupA"] )
        for L,R in conditions:
            AA[1:3, L:R+1] = colours.index('lawngreen')

        #AipB
        conditions = plots_support.ranges_satisfying_condition( A, B, *name_to_edge_condition["AipB"] )
        for L,R in conditions:
            AA[0, L:R+1] = colours.index('lightblue')

        #BipA
        conditions = plots_support.ranges_satisfying_condition( A, B, *name_to_edge_condition["BipA"] )
        for L,R in conditions:
