...
%     Nsega = Naw/Ndiva;  % eg 40,000/20 = 2000 = now the size we use for all Hea calcs  
   ...
   % Save the combined file:
    ...
    for i = 1:Np, 
           ...
           %xcount = xcount + Nsegb;
           for k=1:Ndivb
               % get multiple values of Bblock .. eg Ndivb = 4, 4*250 = 1000
               %   
               %Bstart = Aend + 1;          %((i-1)*Naw)+1;
               Bend   = Bstart + Nsegb - 1;    %(i*Naw);      
               Bblock = Csym(Bstart:Bend);
               
                if (RunType == 'B') || (RunType == 'C')   % eg CSymbol = 'C' , W = end of sentence, char(32)32 = space, 42 = *, 46 = full stop
                       
                     [Heb] = fastentropyblock(Bblock, Nsegb, CSymbol, M, ap, bp, cp);  
                    Hp = [Hp Heb];
                    fprintf('%4.8f \n',Hea(1));
           ...
           Astart = Bend + 1; 
            ...
            Hpa(i) = Hea;
            Hpb(i) = Heb;
            Xh(i) = ia;  % allows for misalignment when invalid block encountered
   end; % end of i Nf
