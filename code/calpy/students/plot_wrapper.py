...
def plot_anomaly(file_name):
    #for 4390.wav
    fs, sound = read_wavfile(file_name) #2,381,072
    pause = dsp.pause_profile(sound, fs) #~29,000
    ...
    for arr in np.array_split(prosody, pause.shape[0] // 10, axis=1):
        ...
        symbols = np.append(symbols, symbolise_speech(arr[0,:], arr[1,:])) #2,976
    ...
    entropy_prof = entropy_profile(symbols, window_size=100) #28 - old data, size=200, overlap=100
    plots.profile_plot(entropy_prof, file_name="classic_anomaly.png",figsize=(10,4))
