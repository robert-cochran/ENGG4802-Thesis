...
def entropy_profile( symbols, window_size = 100, window_overlap = 0, entropy_mode = "classic"):
    ...
    Args:
        ...
        entropy_mode (string, optional): How entropy should be calculated. Defaults to classic, can also be "fast".
    ...
    #     print("key : {}\n cnts: {} ".format(key, cnts))
    #     probs = dict(zip(key,cnts))
            #not sure what this was doing, but it was giving p0(.95),p1(.05) an entropy of 1.55
    #     ent_prof.append(-sum(probs[s] * numpy.log2(probs[s]) for s in window))
    #     for s in window:
    #         print(s)
    #     print()
    #     print("k: {} - entropy value: {} \n".format(k,-sum(probs[s] * numpy.log2(probs[s]) for s in window) ))
    # print("Regular Entropy below: Symbols - " + str(len(symbols)))
    # print(numpy.array(ent_prof))
    # return numpy.array(ent_prof)

    for k in range((N - window_size) // time_step + 1):
        window = symbols[k * time_step : k * time_step + window_size]
        key_event, cnts = numpy.unique(window, return_counts=True)
        cnts = cnts / numpy.sum(cnts)
        #PRINTS THE EVENT AND ITS RATE OF OCCURENCE
        #print("key : {}\n cnts: {} ".format(key_event, cnts))
        probs = dict(zip(key_event,cnts))
        ent_prof.append(-sum(probs[key_event] * numpy.log2(probs[key_event]) for key_event in probs))
        #PRINTS THE WINDOW NUMBER K AND ITS ENTROPY VALUE
        #print("k: {} - entropy value: {} \n".format(k,-sum(probs[s] * numpy.log2(probs[s]) for s in probs)))
    #PRINTS NUMBER OF SYMBOLS SENT
    #print("Regular Entropy below: Symbols - " + str(len(symbols)))
    print(numpy.array(ent_prof))
    return numpy.array(ent_prof)
...
def _pause_pattern_boundrys( pause_A, pause_B, Lcond, Mcond, Rcond ):
    """Compute the left and right boundaries of a particular pause pattern.
